...
# OKX's rubik/stat endpoints (OI and taker-volume) only retain intraday
# granularity for a short rolling window - ~2 days at 5m, ~3-4 days at 1H -
# then return nothing for older dates even though the request is
# well-formed; 1D is available indefinitely. So a fetch merges periods
# finest-first: a coarser period only fills in timestamps a finer one
# didn't cover, so recent hours stay at full resolution and only the
# genuinely out-of-retention hours degrade.
_RUBIK_PERIODS = (("5m", 300_000), ("1H", 3_600_000), ("1D", 86_400_000))
_RUBIK_COVERAGE_THRESHOLD = 0.8


def _rubik_stat(url: str, base_params: dict, start_ts: int, end_ts: int) -> list:
    # Expected bucket count is based on elapsed time up to *now*, not a flat
    # full day, so an in-progress UTC day isn't mistaken for poor retention.
    now_ms = int(pd.Timestamp.now(tz="UTC").timestamp() * 1000)
    span_ms = max(0, min(end_ts, now_ms) - start_ts)

    by_ts = {}
    for period, bucket_ms in _RUBIK_PERIODS:
...
def _taker_volume(symbol: str, date: str, inst_type: str) -> pd.DataFrame:
    ...
    start_ts, end_ts = _day_bounds_ms(date)
    data = _rubik_stat(f"{BASE_URL}/api/v5/rubik/stat/taker-volume",
                        {"ccy": _base_ccy(symbol), "instType": inst_type}, start_ts, end_ts)
    if not data:
        return pd.DataFrame()

    df = pd.DataFrame(data, columns=["open_time", "sell_volume", "buy_volume"])
    df["open_time"] = pd.to_numeric(df["open_time"])
    df = df.set_index("open_time").sort_index()
    df.index = pd.to_datetime(df.index, unit="ms")
    df = df.astype(float)
...
def get_oi(symbol: str, date: str) -> pd.DataFrame:
    ...
    404s on OKX - the rubik stats endpoint is the real source."""
    start_ts, end_ts = _day_bounds_ms(date)
    data = _rubik_stat(f"{BASE_URL}/api/v5/rubik/stat/contracts/open-interest-volume",
                        {"ccy": _base_ccy(symbol)}, start_ts, end_ts)
    if not data:
        return pd.DataFrame()

    df = pd.DataFrame(data, columns=["open_time", "sum_open_interest", "volume"])
    df["open_time"] = pd.to_numeric(df["open_time"])
    df = df.set_index("open_time").sort_index()
    df.index = pd.to_datetime(df.index, unit="ms")
